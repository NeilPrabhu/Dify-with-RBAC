...
from portal.models import Permission
...
class DatasetService:
    ...
    def get_datasets(page, per_page, tenant_id=None, user=None, search=None, tag_ids=None):
        ...
        query = Dataset.query.filter().order_by(Dataset.created_at.desc())
        # TODO fix code below by adding account id and tenant id to datasetPermission table, but what are reprecussions?
        user = None
        if user:
    ...
    def create_empty_dataset(
        ...
        dataset = Dataset(name=name, provider=provider, indexing_technique=indexing_technique)
    ...
    @staticmethod
    def get_dataset_by_name(dataset_name) -> Dataset:
...
class DocumentService:
    ...
    @staticmethod
    def get_user_documents(dataset_id: str, user_id: str):
        # Check if the user has permission to access the dataset
        permission = DatasetPermission.query.filter_by(dataset_id=dataset_id, user_id=user_id, has_permission=True).first()
        if not permission:
            return []

        # Retrieve the documents within the dataset
    ...
    @staticmethod
    def get_batch_documents(dataset_id: str, batch: str, tenant_id: str = None) -> list[Document]:
        if tenant_id:
            documents = (
                db.session.query(Document)
                .filter(
    ...
    def update_document_with_dataset_id(
        ...
        if document_data.get("process_rule"):
            process_rule = document_data["process_rule"]
            # TODO replace account.id with current_user
            if process_rule["mode"] == "custom":
                dataset_process_rule = DatasetProcessRule(
                    dataset_id=dataset.id,
                    mode=process_rule["mode"],
                    rules=json.dumps(process_rule["rules"]),
                    created_by='93add46b-0a95-4d70-be28-bb7c6db7280a',
                )
            elif process_rule["mode"] == "automatic":
                # TODO replace account.id with current_user
                dataset_process_rule = DatasetProcessRule(
