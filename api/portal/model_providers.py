...
model_providers_bp = Blueprint("model_providers_blueprint", __name__)
api = Api(model_providers_bp)
...
class ModelProviderApi(Resource):
    ...
    def post(self, provider: str):
        ...
        try:
            model_provider_service.save_provider_credentials(
                tenant_id=account.current_tenant_id, provider=provider, credentials=args["credentials"]
            )
        except CredentialsValidateFailedError as ex:
    ...
    def delete(self, provider: str):
        ...
        model_provider_service = ModelProviderService()
        model_provider_service.remove_provider_credentials(tenant_id=account.current_tenant_id, provider=provider)
...
class ModelProviderFreeQuotaSubmitApi(Resource):
    ...
    def post(self, provider: str):
        account = get_admin_service_account()
        model_provider_service = ModelProviderService()
        result = model_provider_service.free_quota_submit(tenant_id=account.current_tenant_id, provider=provider)
...
class ModelProviderFreeQuotaQualificationVerifyApi(Resource):
    ...
    def get(self, provider: str):
        ...
        result = model_provider_service.free_quota_qualification_verify(
            tenant_id=account.current_tenant_id, provider=provider, token=args["token"]
        )
